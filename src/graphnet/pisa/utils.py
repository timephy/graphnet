...
def parallel_fit_2D_contour(settings):
    ...
    for i in range(len(settings)):
        ...
        results.append([theta23_value, deltam31_value, result[0]['params'].theta23.value, result[0]
                       ['params'].deltam31.value, result[0]['metric_val'], model_name, id, result[0]['minimizer_metadata']['success']])
    os.makedirs(outdir + '/' + run_name + '/tmp', exist_ok=True)
    results = pd.DataFrame(data=results, columns=['theta23_fixed', 'dm31_fixed',
                           'theta23_best_fit', 'dm31_best_fit', 'mod_chi2', 'model', 'id', 'converged'])
...
def parallel_fit_1D_contour(settings):
    ...
    results = []
    for i in range(len(settings)):
        cfg_path, model_name, outdir, theta23_value, deltam31_value, id, run_name, parameter, fix_all, minimizer_cfg = settings[
            i]
        minimizer_cfg = pisa.utils.fileio.from_file(minimizer_cfg)
        ana = Analysis()
        model = DistributionMaker([cfg_path])
...
def calculate_2D_contours(outdir, run_name, pipeline_path, post_fix='_pred', model_name='gnn', include_retro=True, config_dict=None, grid_size=30, n_workers=10, statistical_fit=False):
    ...
    cfgs = {}
    cfgs[model_name] = make_configs(outdir=outdir, post_fix=post_fix, run_name=run_name,
                                    is_retro=False, pipeline_path=pipeline_path, config_dict=config_dict)
    if include_retro:
        cfgs['retro'] = make_configs(outdir=outdir, post_fix=post_fix, run_name=run_name,
                                     is_retro=True, pipeline_path=pipeline_path, config_dict=config_dict)
    # When sent to workers, booleans can be messed up. Converting to strings are more robust.
    statistical_fit = str(statistical_fit)
    ...
    for model_name in cfgs.keys():
        for i in range(0, grid_size):
            for k in range(0, grid_size):
                settings.append([cfgs[model_name], model_name, outdir, theta23_range[i],
                                dm31_range[k], count, run_name, statistical_fit, minimizer_cfg])
                count += 1
...
def calculate_1D_contours(outdir, run_name, pipeline_path,  post_fix='_pred', model_name='gnn', include_retro=True, config_dict=None, grid_size=30, n_workers=10, statistical_fit=False):
    ...
    for model_name in cfgs.keys():
        for i in range(0, grid_size):
            settings.append([cfgs[model_name], model_name, outdir, theta23_range[i], -1,
                            count, run_name, 'theta23', statistical_fit, minimizer_cfg])
            count += 1
        for i in range(0, grid_size):
            settings.append([cfgs[model_name], model_name, outdir, -1, dm31_range[i],
                            count, run_name, 'deltam31', statistical_fit, minimizer_cfg])
            count += 1
    random.shuffle(settings)
    chunked_settings = np.array_split(settings, n_workers)
